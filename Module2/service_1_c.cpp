...
service_1_C::service_1_C(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::service_1_C)
{
    ui->setupUi(this);
	...
	ui->input_C->resize(250, 50);
	ui->input_C->move(150, 165);
...
service_1_C::~service_1_C()
{
    delete ui;
}

void service_1_C::quitS1_C()
{
    this->close();
}

void service_1_C::on_input_OK_clicked()
{
    int	ret = MSP_SUCCESS;
    ...
    if(flag!=1)
    {
		...
		Text = ui->input_C->text();
        ret = text_to_speech(Text.toLocal8Bit().data(), filename, session_begin_params);
        if (MSP_SUCCESS != ret)
        {
