...
static HANDLE msgqueue_ready_evt = nullptr; /* signaled: the message queque has been created in the thread */
...
static int create_callback_thread(void *thread_proc_para, HANDLE *thread_hdl_out)
{
    ...
    msgqueue_ready_evt = CreateEvent(nullptr, TRUE, FALSE, nullptr);
    if(msgqueue_ready_evt == nullptr)
        return -1;

    rec_thread_hdl = (HANDLE) _beginthreadex(nullptr, 0, record_thread_proc, thread_proc_para, 0, &rec_thread_id);
    if(rec_thread_hdl == nullptr) {
        /* close the event handle */
        CloseHandle(msgqueue_ready_evt);
        msgqueue_ready_evt = nullptr;
        return -1;
    }
...
static void free_rec_buffer(HWAVEIN wi, WAVEHDR *first_header, unsigned headercount)
{
    unsigned int i;
    WAVEHDR *header;
    if(first_header == nullptr || headercount == 0)
        return;
...
static int is_stopped_internal(struct recorder *rec)
{
    ...
    if(header == nullptr || rec->bufcount == 0 /*|| rec->using_flags == NULL*/)
        return 1;
...
{
    if(rec == nullptr || rec->state < RECORD_STATE_READY)
        return;
