...
static void iat_cb(char *data, unsigned long len, void *user_para)
{
    ...
    if(sr == nullptr || sr->ep_stat >= MSP_EP_AFTER_SPEECH)
        return;
...
int sr_init(struct speech_rec * sr, const char * session_begin_params, enum sr_audsrc aud_src, int devid, struct speech_rec_notifier * notify)
{
    ...
    if (session_begin_params == nullptr) {
    ...
    if (sr->session_begin_params == nullptr) {
    ...
    if (aud_src == SR_MIC) {
        ...
        if (sr->recorder == nullptr || errcode != 0) {
...
fail:
    ...
    if (sr->session_begin_params) {
        ...
        sr->session_begin_params = nullptr;
    }
...
int sr_stop_listening(struct speech_rec *sr)
{
    ...
    sr->state = SR_STATE_INIT;
    ret = QISRAudioWrite(sr->session_id, nullptr, 0, MSP_AUDIO_SAMPLE_LAST, &sr->ep_stat, &sr->rec_stat);
    if (ret != 0) {
	...
	while (sr->rec_stat != MSP_REC_STATUS_COMPLETE) {
		...
		//if (rslt&&strlen(rslt) <= 2)	rslt = nullptr;
		if (nullptr != rslt && sr->notif.on_result)
			sr->notif.on_result(rslt, sr->rec_stat == MSP_REC_STATUS_COMPLETE ? 1 : 0);
